...
static void add_to_proxy_exclude(const char *str)
{
	char *e;
	char *s, *orgs;
	...
	orgs = s = xstrdup(str);
	while((e = strqsep(&s, ':')) != 0) {
		if(list_search(gvProxyExclude, (listsearchfunc)strcmp, e) == 0)
			list_additem(gvProxyExclude, xstrdup(e));
	}
	xfree(orgs);
}
